...
import id.dni.pvim.ext.service.PVLoginService;
...
public class UserOperation {
    ...
    public PVLoginResponse login(PVLoginRequest request) {
        ...
        PVLoginService loginService = new PVLoginService();
        ProviewLoginRequest pvlReq = new ProviewLoginRequest();
        pvlReq.setPassword(request.getPassword());
        pvlReq.setUsername(request.getUsername());
        pvlReq.setTimeout(Integer.parseInt(PatcherConfig.getInstance().get("LoginService.connect_timeout", "3000")));
        pvlReq.setUrl(PatcherConfig.getInstance().get("LoginService.url"));
        ProviewLoginResponse pvlResp = loginService.login(pvlReq);
        
        PVLoginResponse response = new PVLoginResponse();
        response.setIsLoginSuccess(pvlResp.isSuccess());
