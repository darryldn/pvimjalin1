/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package id.dni.pvim.ext.dto;

import id.dni.pvim.ext.web.in.OperationError;
import java.util.List;

/**
 *
 * @author darryl.sulistyan
 */
public class PvWsCassetteResponse {
    private OperationError err;
    private List<PvWsCassette> response;

    public OperationError getErr() {
        return err;
    }

    public void setErr(OperationError err) {
        this.err = err;
    }

    public List<PvWsCassette> getResponse() {
        return response;
    }

    public void setResponse(List<PvWsCassette> response) {
        this.response = response;
    }
}
